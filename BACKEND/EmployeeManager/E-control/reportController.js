...
async function generateComprehensivePDF(reportData, res) {
  ...
  // Add professional company logo - exactly like Animal Management
  // Create a professional logo design that matches the real logo
  doc.setFillColor(...primaryColor);
  doc.rect(20, 15, 25, 25, 'F');
  
  // Add white circle in center (like the real logo)
  doc.setFillColor(255, 255, 255);
  doc.circle(32.5, 27.5, 8, 'F');
  
  // Add "MOF" text in the circle
  doc.setTextColor(...primaryColor);
  doc.setFontSize(8);
  doc.setFont('helvetica', 'bold');
  doc.text('MOF', 32.5, 29.5, { align: 'center' });
