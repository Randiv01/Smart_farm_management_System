...
const FertilizerStock = () => {
  ...
  const [exportModal, setExportModal] = useState(getExportModalConfig('fertilizerStock'));
  ...
  const handleExport = () => {
    const dataToExport = exportModal.selection === 'current' ? filteredFertilizers : fertilizers;
    ...
    const config = {
      ...EXPORT_CONFIGS.fertilizerStock,
      dataFormatter: (fertilizer) => {
        const days = calculateDaysUntilExpiry(fertilizer.expiryDate);
        return {
          'Fertilizer Name': fertilizer.name || 'N/A',
          'Quantity': fertilizer.quantity || 0,
          'Remaining': fertilizer.remaining || 0,
          'Unit': fertilizer.unit || 'N/A',
          'Type': fertilizer.fertilizerType || 'N/A',
          'Expiry Date': fertilizer.expiryDate ? new Date(fertilizer.expiryDate).toLocaleDateString() : 'N/A',
                ...
                <div className="space-y-2">
                  <label className={`flex items-center ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
                    <input
                      ...
                      className="mr-2"
                    />
                    Current View ({filteredFertilizers.length} items)
                  </label>
                  <label className={`flex items-center ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
                    <input
                      type="radio"
                      ...
                      className="mr-2"
                    />
                    All Data ({fertilizers.length} items)
                  </label>
                </div>
              </div>
            </div>
