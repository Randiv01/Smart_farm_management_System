...
const AnimalFoodStock = () => {
  ...
  const [exportModal, setExportModal] = useState(getExportModalConfig('animalFoodStock'));
  ...
  const handleExport = () => {
    const dataToExport = exportModal.selection === 'current' ? filteredAnimalFoods : animalFoods;
    ...
    const config = {
      ...EXPORT_CONFIGS.animalFoodStock,
      dataFormatter: (food) => {
        const days = calculateDaysUntilExpiry(food.expiryDate);
        return {
          'Food Name': food.name || 'N/A',
          'Quantity': food.quantity || 0,
          'Remaining': food.remaining || 0,
          'Unit': food.unit || 'N/A',
          'Target Animal': food.targetAnimal || 'N/A',
          'Shelf Life': food.shelfLife || 'N/A',
          'Expiry Date': food.expiryDate ? new Date(food.expiryDate).toLocaleDateString() : 'N/A',
                ...
                <div className="space-y-2">
                  <label className={`flex items-center ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
                    <input
                      ...
                      className="mr-2"
                    />
                    Current View ({filteredAnimalFoods.length} items)
                  </label>
                  <label className={`flex items-center ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
                    <input
                      type="radio"
                      ...
                      className="mr-2"
                    />
                    All Data ({animalFoods.length} items)
                  </label>
                </div>
              </div>
            </div>
