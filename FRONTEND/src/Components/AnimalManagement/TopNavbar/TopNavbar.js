...
import { MenuIcon, BellIcon, ChevronDownIcon, UserIcon, SunIcon, MoonIcon, LogOutIcon } from "lucide-react";
...
const TopNavbar = ({ onMenuClick, sidebarOpen }) => {
  ...
  const getProfileImageUrl = (path) => {
    ...
    // For locally stored paths, clean them up
    ...
    >
      <div className="flex items-center justify-between w-full">
        ...
        <div className="flex items-center">
          ...
          <h2 className={`text-lg md:text-xl font-semibold truncate ${darkMode ? "text-white" : "text-gray-800"}`}>
            {t("dashboard.title", { defaultValue: `${formatRole(userData.role)} Dashboard` })}
          </h2>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="relative">
            ...
            {notificationsOpen && (
              <div className={`absolute right-0 mt-2 w-80 rounded-md shadow-lg py-1 z-50 ${
            ...
            {userMenuOpen && (
              <div className={`absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 z-50 ${
                ...
                <div className="px-4 py-2 border-b border-gray-200 dark:border-gray-700 flex items-center">
                  <div className={`rounded-full overflow-hidden w-10 h-10 flex items-center justify-center mr-3 ${
                    darkMode ? "bg-gray-700" : "bg-gray-200"
                  }`}>
                    {userData.profileImage ? (
                      <img 
                      ...
                      <UserIcon size={20} className={darkMode ? "text-gray-200" : "text-gray-600"} />
                    )}
                  </div>
                  <div>
                    <p className={`text-sm font-medium ${darkMode ? "text-white" : "text-gray-900"}`}>
