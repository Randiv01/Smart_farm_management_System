...
const LoginPage = () => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      switch(role) {
        case "animal": navigate("/AnimalManagement"); break;
        case "plant": navigate("/PlantManagement"); break;
        case "inv": navigate("/InventoryManagement"); break;
        case "emp": navigate("/EmployeeManagement"); break;
        case "health": navigate("/admin/dashboard"); break;
        case "owner": navigate("/OwnerDashboard"); break;
        default: navigate("/"); break;
      }

    } catch (err) {
      setError(err?.response?.data?.error || err?.response?.data?.message || err?.message || "Login failed. Please check your credentials and try again.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="w-full md:w-1/3 bg-gradient-to-br from-dark-green to-green-900 text-soft-white p-10 flex flex-col justify-center relative overflow-hidden">
          ...
          <div className="relative z-10 text-center">
            <div className="w-20 h-20 bg-soft-white bg-opacity-10 rounded-full flex items-center justify-center mx-auto mb-6">
              <svg className="w-10 h-10 text-soft-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="space-y-3 text-left">
              {["Bulk order animal and plant products", "Track your past orders and deliveries", "Get updates on seasonal harvests & offers"].map((text, idx) => (
